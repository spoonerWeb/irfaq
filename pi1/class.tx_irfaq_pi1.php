 ...
 * @author Netcreators <extensions@netcreators.com>
 */
class tx_irfaq_pi1 extends AbstractPlugin
{
    // Same as class name
    public $prefixId = 'tx_irfaq_pi1';
    // Path to this script relative to the extension dir.
    public $scriptRelPath = 'pi1/class.tx_irfaq_pi1.php';
    public $extKey = 'irfaq';
     ...
     * @var array
     */
    protected $content_languages = array(0);
    ...
    function init($conf)
    {
        ...
        $this->content_languages = array($this->getTypoScriptFrontendController()->sys_language_uid);
    ...
    {
        $storagePid = $this->getTypoScriptFrontendController()->getStorageSiterootPids();
    ...
    function searchView()
    {
        ...
        $formURL = htmlspecialchars(
            $this->pi_linkTP_keepPIvars_url(array('cat' => null), 0, 1, $this->conf['searchPid'])
        );
        ...
        if (!empty($this->faqCount)) {
            if ($this->conf['addDefaultJs'] == 1) {
                ...
                $header .= '<script type="text/javascript" src="' .
                    ExtensionManagementUtility::siteRelPath($this->extKey) . 'res/toggleFaq.js"></script>';
                $this->getTypoScriptFrontendController()->additionalHeaderData['tx_irfaq'] = $header;
            }
    ...
    function getSelectConf($where)
    {
        ...
        $selectConf['where'] .= ' AND tx_irfaq_q.sys_language_uid=' . intval(
                $this->getTypoScriptFrontendController()->config['config']['sys_language_uid']
            );
    ...
    function getRelatedEntries($list)
    {
        ...
        if (count($list)) {
            ...
            if (is_array($rows)) {
                ...
                $content = $this->cObj->substituteMarkerArrayCached(
                    $template,
                    array('###TEXT_RELATED_FAQ###' => $this->pi_getLL('text_related_faq')),
                    array('###RELATED_FAQ_ENTRY###' => $content)
                );
            }
        }
        return $content;
    ...
    function getRelatedLinks($list)
    {
        ...
        if (count($list)) {
            ...
            $content = $this->cObj->substituteMarkerArrayCached(
                $template,
                array('###TEXT_RELATED_LINKS###' => $this->pi_getLL('text_related_links')),
                array('###RELATED_LINK_ENTRY###' => $content)
            );
        }
        return $content;
    }
    ...
    function singleView()
    {
        ...
        if (count($rows) == 0) {
            ...
            $markers['###BACK_HREF###'] = $this->cObj->typoLink_URL(
                array('parameter' => $this->getTypoScriptFrontendController()->id)
            );
    ...
    function addHeaderParts()
    {
        ...
        if (!isset($this->getTypoScriptFrontendController()->additionalHeaderData['tx_irfaq_pi1_' . $subpart])) {
            $template = $this->cObj->getSubpart($this->templateCode, $subpart);
            $this->getTypoScriptFrontendController()->additionalHeaderData['tx_irfaq_pi1_' . $subpart] =
                $this->cObj->substituteMarker(
                    $template,
                    '###SITE_REL_PATH###',
                    ExtensionManagementUtility::siteRelPath('irfaq')
                );
        }
    }
